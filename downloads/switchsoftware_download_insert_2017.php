...
for($i = 2; $i <= 150; $i++) {
  // 備考欄の取得を省く
  if(strpos($doc['table tbody tr:eq(' . $i . ') td:eq(0)']->text(), 'エディション') !== false) {
    $remove_i[$remove_i_count++] = $i;
    continue;
  }
...
for($i = 2; $i <= 150; $i++) {
  ...
  if(strpos($doc['table tbody tr:eq(' . $i . ') td:eq(5)']->text(), "教") !== false || strpos($doc['table tbody tr:eq(' . $i . ') td:eq(5)']->text(), "+") !== false || strpos($doc['table tbody tr:eq(' . $i . ') td:eq(7)']->text(), "審査") !== false) {
